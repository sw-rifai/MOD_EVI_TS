getHdf <- function(product, begin = NULL, end = NULL, tileH = NULL, tileV = NULL, 
  ...
  if (!missing(HdfName)) {
    ...
    for (z in seq_along(product$PRODUCT)) {
      if (product$TYPE[z] == "Swath") {
        ...
        todo <- paste0(product$PRODUCT[z], ".", product$CCC[[which(names(product$CCC) == 
                                                                     product$PRODUCT[z])]])
        for (u in seq_along(todo)) {
          ...
          onlineInfo <- getStruc(product = product$PRODUCT[z], 
                                 collection = product$CCC[[which(names(product$CCC) == 
                                                                   product$PRODUCT[z])]], server = opts$MODISserverOrder[1], 
          ...
          if (!is.na(onlineInfo$online)) {
            if (!onlineInfo$online & length(opts$MODISserverOrder) == 
              ...
              onlineInfo <- getStruc(product = product$PRODUCT[z], 
                                     collection = product$CCC[[which(names(product$CCC) == 
                                                                       product$PRODUCT[z])]], begin = tLimits$begin, 
